...
ProgramType::ProgramType(const ParameterType& inputType, int inputLength, const ParameterType& outputType, int outputLength)
	: inputType(inputType), inputLength(inputLength), outputType(outputType), outputLength(outputLength) {
}
...
void ProgramType::read(istream& is) {
	...
	inputLength = std::get<1>(t);
	outputType = std::get<2>(t);
	outputLength = std::get<3>(t);
}

void ProgramType::write(ostream& os) const {
	tuple<ParameterType, int, ParameterType, int> t(inputType, inputLength, outputType, outputLength);
	os << t;
}
...
int ProgramType::getInputLength() const {
	return inputLength;
}
...
int ProgramType::getOutputLength() const {
	return outputLength;
}

vector<double> ProgramType::clipInput(const vector<double>& input) const {
	vector<double> result(inputLength, 0);
...
vector<double> ProgramType::clipOutput(const vector<double>& output) const {
	vector<double> result(outputLength, 0);
